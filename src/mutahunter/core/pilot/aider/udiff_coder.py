...
# For more information, see https://github.com/paul-gauthier/aider
...
class UnifiedDiffCoder:
    ...
    def apply_edits(self, edits, original_code):
        success_edits = []
        failed_edits = []
        ...
        for path, hunk in uniq:
            ...
            try:
                content = do_replace(full_path, original_code, hunk)
            ...
            success_edits.append((path, hunk, content))

        # if errors:
        #     errors = "\n\n".join(errors)
        #     if len(errors) < len(uniq):
        #         errors += "Note: some hunks did apply successfully. See the updated source code shown above.\n\n"
        #     raise ValueError(errors)

        return success_edits, failed_edits

    def apply_edits_for_unittest_generation(self, edits):
        ...
        for path, hunk in edits:
            ...
            uniq.append((path, hunk))
        # store updated content for each file
        for path, hunk in uniq:
            full_path = self.abs_root_path(path)
    ...
    def return_error_count(self):
        error_count = len(self.errors)
        self.errors = []  # reset the error count
        return error_count
